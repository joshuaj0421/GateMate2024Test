...
gateRouter.put("/status/:fieldId/:gateId", async (req: Request, res: Response) => {
  try {
    if (!req.session?.user) return res.status(403).send("User not logged in");
    ...
    if (actualWaterLevel !== undefined) gate.actualWaterLevel = actualWaterLevel;
    if (connectionError !== undefined) gate.connectionError = connectionError;
    if (lowBattery !== undefined) gate.lowBattery = lowBattery;
