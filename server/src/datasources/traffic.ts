...
async function generateTraffic(email: string, fieldId: number) {
  ...
  if (!field) return { message: "Field not found", status: "404" };
  ...
  for (let gateId = 1; gateId <= numGates; gateId++) {
    ...
    if (!currGate) return { message: "Gate not found", status: "404" };
...
async function getTraffic(userId: string, fieldId: number) { // redundent
